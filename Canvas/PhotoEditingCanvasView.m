...
- (instancetype)init
{
    if ((self = [super initWithFrame:CGRectZero])) {
        ...
        self.opaque = NO;
        self.tool = [[PKInkingTool alloc] initWithInkType:PKInkTypeMarker color:[UIColor blackColor] width:STANDARD_LINE_WIDTH];
...
- (void)updateToolWithCurrentZoomScale:(CGFloat)currentZoomScale {
    self.tool = [[PKInkingTool alloc] initWithInkType:PKInkTypeMarker color:[UIColor blackColor] width:[self adjustedLineWidthForZoomScale:currentZoomScale]];
}
