...
class ToolPickerItem: NSMenuToolbarItem {
    ...
    init(delegate: ToolPickerItemDelegate) {
        ...
        label = Strings.ToolPickerItem.itemLabel
        itemMenu = currentMenu
    }

    private var currentMenu: UIMenu {
        UIMenu(
            title: Strings.ToolPickerItem.menuTitle,
            children: HighlighterTool.allCases.map {
                Command(tool: $0, currentTool: delegate.highlighterTool)
            }
        )
    }
    ...
    private class Command: UICommand {
        ...
        static func title(for tool: HighlighterTool) -> String {
            switch tool {
            case .magic: Strings.ToolPickerItem.magicToolItem
            case .lasso: Strings.ToolPickerItem.lassoToolItem
            case .manual: Strings.ToolPickerItem.manualToolItem
            case .eraser: Strings.ToolPickerItem.eraserToolItem
            }
        }
