...
public class PurchaseMarketingHostingController: UIHostingController<PurchaseMarketingView> {
    private var purchaseState: PurchaseState {
        didSet {
            rootView = PurchaseMarketingView(purchaseState: .init(
    ...
    public init(
        ...
        super.init(rootView: PurchaseMarketingView(purchaseState: initialBinding))

        rootView = PurchaseMarketingView(purchaseState: Binding<PurchaseState>(
            get: { self.purchaseState },
            set: { self.purchaseState = $0 }
        ))
